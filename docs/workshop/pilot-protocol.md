# Morgan classroom pilot protocol (E2-021)

## Goal

Validate ≥80% save rate in a 5-user workshop dry-run before v1.0.0.

## Prerequisites

- `pixelanea_<version>_amd64.deb` installed on each machine (`sudo apt install ./pixelanea_*.deb`)
- Optional: `sudo apt install zenity` for native File Open/Save dialogs
- `docs/workshop/teacher-guide.md` printed or projected
- Template project: `docs/examples/projects/blank-starter.pixelanea` (copy per student; see [templates/README.md](./templates/README.md))

## Session (45 min)

1. **Install** (5 min) — open **Pixelanea** from the app menu (or run `pixelanea`), confirm status bar shows *Ready*
2. **Draw** (10 min) — 32×32 character, 3+ colors
3. **Save** (10 min) — Save As to `~/pixelanea-workshop/{name}.pixelanea`
4. **Reopen** (5 min) — Open saved file; verify pixels intact
5. **Export** (10 min) — File → Export → spritesheet; confirm download
6. **Survey** (5 min) — Did save work without help? (Y/N)

## Success metric

≥4 of 5 participants complete step 3 without instructor intervention (80%).

## Record results

| Participant | Save without help | Reopen OK | Export OK | Notes |
|-------------|-------------------|-----------|-----------|-------|
| 1 | | | | |
| 2 | | | | |
| 3 | | | | |
| 4 | | | | |
| 5 | | | | |
