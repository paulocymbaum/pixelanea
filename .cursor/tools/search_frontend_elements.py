#!/usr/bin/env python3
"""Search Pixelanea frontend layers for code elements with similar names.

Agents call this tool to locate components, hooks, stores, tools, and other
symbols inside a specific `apps/web/src` layer without guessing paths.

Usage:
  python .cursor/tools/search_frontend_elements.py --layer shell AppHeader LeftToolRail
  python .cursor/tools/search_frontend_elements.py --layer state --queries editorStore undo
  python .cursor/tools/search_frontend_elements.py --layer state --queries '["editorStore","undo"]'
  python .cursor/tools/search_frontend_elements.py --list-layers
"""
...
# Layer name → path relative to the repo root (see PRACTICES.md / frontend skill).
FRONTEND_LAYERS: dict[str, str] = {
    "pages": "apps/web/src/pages",
    "shell": "apps/web/src/shell",
    "components": "apps/web/src/components",
    "ui": "apps/web/src/components/ui",
    "toolbar": "apps/web/src/components/toolbar",
    "palette": "apps/web/src/components/palette",
    "frames": "apps/web/src/components/frames",
    "animation": "apps/web/src/components/animation",
    "project": "apps/web/src/components/project",
    "import": "apps/web/src/components/import",
    "onboarding": "apps/web/src/components/onboarding",
    "canvas": "apps/web/src/canvas",
...
CODE_ELEMENT_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    (re.compile(r"^\s*export\s+default\s+function\s+(\w+)"), "default-export"),
    (re.compile(r"^\s*export\s+default\s+class\s+(\w+)"), "default-export"),
    ...
    (re.compile(r"^\s*export\s+\{\s*([^}]+)\}"), "named-export"),
    (re.compile(r"^\s*function\s+(\w+)"), "function"),
    (re.compile(r"^\s*const\s+(\w+)\s*="), "const"),
...
CSS_VARIABLE_PATTERN = re.compile(r"--([a-zA-Z0-9-]+)")
...
def normalize_layer_name(raw: str) -> str:
    ...
    key = LAYER_ALIASES.get(key, key)
    return key


def resolve_layer_path(layer: str, root: Path) -> tuple[str, Path]:
    normalized = normalize_layer_name(layer)
    if normalized not in FRONTEND_LAYERS:
        supported = ", ".join(sorted(FRONTEND_LAYERS))
        raise ValueError(
            f"Unknown frontend layer {layer!r}. Supported layers: {supported}"
        )
    relative = FRONTEND_LAYERS[normalized]
...
def split_named_exports(export_block: str) -> list[str]:
    names: list[str] = []
...
def extract_elements_from_file(path: Path, repo_root: Path) -> list[CodeElement]:
    ...
    for line_number, line in enumerate(lines, start=1):
        for pattern, kind in CODE_ELEMENT_PATTERNS:
            match = pattern.search(line)
            if not match:
                continue

            if kind == "named-export":
                for name in split_named_exports(match.group(1)):
                    elements.append(
                        CodeElement(
                            name=name,
                            kind=kind,
        ...
        if path.suffix == ".css":
            for css_match in CSS_VARIABLE_PATTERN.finditer(line):
                token = css_match.group(1)
                elements.append(
                    CodeElement(
                        name=token,
                        kind="css-variable",
    ...
    if len(raw_queries) == 1:
        ...
        if candidate.startswith("["):
            ...
            if not isinstance(parsed, list) or not all(isinstance(item, str) for item in parsed):
                raise ValueError("--queries JSON value must be an array of strings")
            return parsed
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Search a Pixelanea frontend layer for similarly named code elements.",
    )
    parser.add_argument(
        "--layer",
        help="Frontend layer name (e.g. shell, canvas, state, ui, toolbar).",
    )
    parser.add_argument(
        "queries",
        nargs="*",
    ...
    parser.add_argument(
        "--list-layers",
        action="store_true",
        help="Print supported frontend layers and exit.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.list_layers:
        payload = {
            "layers": {
                name: FRONTEND_LAYERS[name] for name in sorted(FRONTEND_LAYERS)
            },
